...
private:
    ...
    using ALUFunc = std::function<void(u8&)>;
    using RotShiftFunc = std::function<void(u8&)>;

    ALUFunc getAlu(u8 y);
    RotShiftFunc getRotateShift(u8 y);

    void decodeInstruction(u8 opCode, u8& x, u8& y, u8& z, u8& p, u8& q);
    u8 fetchInstruction(i64& tState);

    void execute(u8 opCode, i64& tState);
    void executeDDFDCB(Reg& idx, i64& tState);
    void executeDDFD(Reg& idx, i64& tState);
    void executeED(i64& tState);
...
#define PEEK(a) m_memory.peek((a), tState)
#define POKE(a, b) m_memory.poke((a), (b), tState)
#define PEEK16(a) m_memory.peek16((a), tState)
#define POKE16(a, w) m_memory.poke16((a), (w), tState)
#define CONTEND(a, t, n) m_memory.contend((a), (t), (n), tState)
    ...
    case 3:
        ...
        case 3:
            ...
            case 2:     // D3 - OUT (n),A()       A() -> $AAnn
                d = PEEK(PC());
                m_io.out((u16)d | ((u16)A() << 8), A(), tState);
                m_mp.h = A();
                m_mp.l = (u8)(d + 1);
                ++PC();
                break;
