...
optional<ExprValue> ExpressionEvaluator::parseExpression(Lex& lex, ErrorManager& errs, const Memory& memory, const Lex::Element*& e, MemAddr currentAddress)
{
    Expression expr = constructExpression(e);
    return eval(lex, errs, memory, currentAddress, expr);
}
...
optional<ExprValue> ExpressionEvaluator::eval(Lex& lex, ErrorManager& errs, const Memory& memory, MemAddr currentAddress, Expression& expr)
{
...
#define FAIL()                                                          \
    ...
    for (const auto& v : output)
    {
        ...
        case Expression::ValueType::BinaryOp:
            ...
            case T::Minus:      stack.emplace_back(ExprValue::opSub(memory, a, b)); break;
...
struct SubExpr
{
    SubExpr(const Memory& memory)
        : m_memory(memory)
    {}

    ExprValue operator() (i64 x, i64 y) { return ExprValue(x - y); }
    ExprValue operator() (MemAddr a, i64 x) { return ExprValue(a - (int)x); }
    ExprValue operator() (i64 x, MemAddr a)
    {
        optional<u16> z80Addr = m_memory.convert(a);
        return z80Addr ? ExprValue(x - (int)*z80Addr) : ExprValue(x);
    }
...
ExprValue ExprValue::opSub(const Memory& memory, const ExprValue& a, const ExprValue& b)
{
    return visit(SubExpr(memory), a.m_value, b.m_value);
}
